...
export default async (req, res) => {
  ...
  if (batch !== "3") {
    ...
    streamsMap = addresses.nodesMap.reduce((acc, val, index) => {
      return Object.assign({}, acc, { [index]: val.ethAddress });
    }, {});
    streamMap = addresses.nodesMap.reduce((acc, val, index) => {
      return Object.assign({}, acc, {
        [index]: {
          content: val.hoprAddresses.reduce(
            (_acc, _val) => Object.assign({}, _acc, { [_val]: val.ethAddress }),
            {}
          ),
        },
      });
